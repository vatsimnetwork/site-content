...
* **A7** A member shall only use software approved by VATSIM to connect to, or interface with the network. [Approved Software](/policy/approved-software). A member shall become familiar with the basic use of these programs prior to logging on to the VATSIM network.
...
* **C4** VATSIM recognizes the following suffixes for air traffic control positions. A member shall only use the corresponding facility type when connecting.
  ...
  * **(g)** FSS - Flight Service Station. __NOTE__: Approved Flight Service Stations are specified in Appendix B of the VATSIM [Global Ratings Policy](/policy/global-ratings-policy)
...
* **C6** Only approved positions and their corresponding frequencies may be opened by controllers. Controllers may only open a position they are authorized to, in accordance with global and local policy. See the [Global Ratings Policy](/policy/global-ratings-policy) and the [Transfer and Visiting Controllers Policy](/policy/transfer-and-visiting-controller-policy) for more information.
